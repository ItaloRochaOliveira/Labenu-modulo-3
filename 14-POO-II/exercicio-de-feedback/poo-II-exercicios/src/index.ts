...
import { UpdateVideoInputDTO, VideosDB } from "./types";
...
app.get("/videos", async (req: Request, res: Response) => {
  try {
    const video = new VideoDatabase();
    const videosDB = await video.findVideo();
...
app.post("/videos", async (req: Request, res: Response) => {
  try {
    ...
    const video = new VideoDatabase();
    const videoExist = await video.findVideoById(id);
    ...
    const videoDB = {
      ...
      created_at: newVideo._createdAt,
    };

    await video.createVideo(videoDB);
...
app.put("/videos/:id", async (req: Request, res: Response) => {
  try {
    ...
    const { title, duration } = req.body as UpdateVideoInputDTO;
    ...
    const videoExist = await videoDB.findVideoById(id);
    ...
    // const updateVideDB = {
    //   id: video._id || videoExist.id,
    //   title: video._title || videoExist.title,
    //   duration: video._duration || videoExist.duration,
    //   created_at: video._createdAt,
    // };

    // await videoDB.updateVideo(updateVideDB, id);

    const updateVideo = new Video(
      videoExist.id,
      videoExist.title,
      videoExist.duration,
      videoExist.created_at
    );
    ...
    await videoDB.updateVideo(updateVideoDB, id);

    res.status(200).send({ message: "Atualizado com sucesso", updateVideoDB });
  } catch (error) {
    console.log(error);
...
app.delete("/videos/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (!videoExist) {
      res.status(400);
      throw new Error("Video não encontrado para exclusão.");
    }
