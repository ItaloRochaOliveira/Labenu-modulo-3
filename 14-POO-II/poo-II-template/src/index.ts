...
app.post("/users", async (req: Request, res: Response) => {
  try {
    ...
    await userDB.createUser(newUserDB);

    res.status(201).send({ message: "Criado com sucesso", newUserDB });
  } catch (error) {
    console.log(error);
...
app.get("/accounts", async (req: Request, res: Response) => {
  try {
    const returnAccounts = new AccountDatabase();

    const accountsDB = await returnAccounts.findUser();
...
app.get("/accounts/:id/balance", async (req: Request, res: Response) => {
  try {
    ...
    const returnAccount = new AccountDatabase();
    const accountDB: TAccountDB | undefined = await returnAccount.findUserById(
      id
    );
...
app.post("/accounts", async (req: Request, res: Response) => {
  try {
    ...
    const accountDBExists: TAccountDB | undefined =
      await returnAccount.findUserById(id);
    ...
    await returnAccount.createUser(newAccountDB);

    res.status(201).send({ message: "funcionou", newAccount });
  } catch (error) {
    console.log(error);
...
app.put("/accounts/:id/balance", async (req: Request, res: Response) => {
  try {
    ...
    await returnAccount.updateBalance(newBalance, accountDBExists.id);
